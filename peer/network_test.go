...
var (
	defaultPeerVersion = version.NewDefaultApplication("dijethhtest", 1, 0, 0)
...
func TestRequestMinVersion(t *testing.T) {
	...
	requestBytes, err := message.RequestToBytes(codecManager, requestMessage)
	assert.NoError(t, err)
	assert.NoError(t, net.Connected(nodeID, version.NewDefaultApplication("dijethhtest", 1, 7, 1)))

	// ensure version does not match
	responseBytes, failed, err := client.RequestAny(version.NewDefaultApplication("dijethhtest", 2, 0, 0), requestBytes)
	assert.Equal(t, err.Error(), "no peers found matching version dijethhtest/2.0.0 out of 1 peers")
	assert.True(t, failed)
	assert.Nil(t, responseBytes)

	// ensure version matches and the request goes through
	responseBytes, failed, err = client.RequestAny(version.NewDefaultApplication("dijethhtest", 1, 0, 0), requestBytes)
	assert.NoError(t, err)
	assert.False(t, failed)
