...
type Mempool struct {
	...
	// DJTXAssetID is the fee paying currency of any atomic transaction
	DJTXAssetID ids.ID
...
func NewMempool(DJTXAssetID ids.ID, maxSize int) *Mempool {
	return &Mempool{
...
// amount of [DJTXAssetID] given the value of [gasUsed].
func (m *Mempool) atomicTxGasPrice(tx *Tx) (uint64, error) {
	...
	burned, err := tx.Burned(m.DJTXAssetID)
	if err != nil {
		return 0, err
	}
