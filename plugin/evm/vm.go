...
import (
	...
	"github.com/lasthyphen/dijethh/plugin/evm/message"

	avalanchegoMetrics "github.com/lasthyphen/dijetsgogo/api/metrics"
	dijethh "github.com/lasthyphen/dijethh/chain"
	"github.com/lasthyphen/dijethh/consensus/dummy"
	"github.com/lasthyphen/dijethh/core"
	"github.com/lasthyphen/dijethh/core/state"
	"github.com/lasthyphen/dijethh/core/types"
	"github.com/lasthyphen/dijethh/eth/ethconfig"
	dijethhPrometheus "github.com/lasthyphen/dijethh/metrics/prometheus"
	"github.com/lasthyphen/dijethh/node"
	"github.com/lasthyphen/dijethh/params"
	"github.com/lasthyphen/dijethh/peer"
	"github.com/lasthyphen/dijethh/rpc"
	...
	// inside of cmd/geth.
	_ "github.com/lasthyphen/dijethh/eth/tracers/js"
	_ "github.com/lasthyphen/dijethh/eth/tracers/native"
	...
	"github.com/lasthyphen/dijetsgogo/cache"
	...
	"github.com/lasthyphen/dijetsgogo/database/prefixdb"
	...
	"github.com/lasthyphen/dijetsgogo/utils/math"
	"github.com/lasthyphen/dijetsgogo/utils/perms"
	"github.com/lasthyphen/dijetsgogo/utils/profiler"
	"github.com/lasthyphen/dijetsgogo/utils/timer/mockable"
	"github.com/lasthyphen/dijetsgogo/vms/components/djtx"
	"github.com/lasthyphen/dijetsgogo/vms/components/chain"
	"github.com/lasthyphen/dijetsgogo/vms/secp256k1fx"

	commonEng "github.com/lasthyphen/dijetsgogo/snow/engine/common"

	avalancheJSON "github.com/lasthyphen/dijetsgogo/utils/json"
)
...
func (vm *VM) Initialize(
	...
	if b, err := json.Marshal(vm.config); err == nil {
		log.Info("Initializing Dijethh VM", "Version", Version, "Config", string(b))
	} else {
		// Log a warning message since we have already successfully unmarshalled into the struct
		log.Warn("Problem initializing Dijethh VM", "Version", Version, "Config", string(b), "err", err)
	}
	...
	ethChain, err := dijethh.NewETHChain(&ethConfig, &nodecfg, vm.chaindb, vm.config.EthBackendSettings(), vm.createConsensusCallbacks(), lastAcceptedHash, &vm.clock)
	if err != nil {
		return err
	}
	vm.chain = ethChain
	...
	if metrics.Enabled {
		gatherer := dijethhPrometheus.Gatherer(metrics.DefaultRegistry)
...
func (vm *VM) preBatchOnFinalizeAndAssemble(header *types.Header, state *state.StateDB, txs []*types.Transaction) ([]byte, *big.Int, *big.Int, error) {
	for {
		...
		if rules.IsApricotPhase4 {
			contribution, gasUsed, err = tx.BlockFeeContribution(rules.IsApricotPhase5, vm.ctx.DJTXAssetID, header.BaseFee)
			if err != nil {
...
func (vm *VM) postBatchOnFinalizeAndAssemble(header *types.Header, state *state.StateDB, txs []*types.Transaction) ([]byte, *big.Int, *big.Int, error) {
	...
	for {
		...
		// ApricotPhase5.
		txContribution, txGasUsed, err = tx.BlockFeeContribution(true, vm.ctx.DJTXAssetID, header.BaseFee)
		if err != nil {
			return nil, nil, nil, err
		}
...
func (vm *VM) onExtraStateChange(block *types.Block, state *state.StateDB) (*big.Int, *big.Int, error) {
	...
	for _, tx := range txs {
		...
		if isApricotPhase4 {
			contribution, gasUsed, err := tx.BlockFeeContribution(isApricotPhase5, vm.ctx.DJTXAssetID, block.BaseFee())
			if err != nil {
				return nil, nil, err
			}
...
func (vm *VM) CreateHandlers() (map[string]*commonEng.HTTPHandler, error) {
	...
	if vm.config.DijethhAdminAPIEnabled {
		adminAPI, err := newHandler("admin", NewAdminService(vm, os.ExpandEnv(fmt.Sprintf("%s_dijethh_performance_%s", vm.config.DijethhAdminAPIDir, primaryAlias))))
		if err != nil {
			return nil, fmt.Errorf("failed to register service for admin API due to %w", err)
		}
		apis[adminEndpoint] = adminAPI
		enabledAPIs = append(enabledAPIs, "dijethh-admin")
	}
