...
import (
	...
	"github.com/lasthyphen/dijethh/trie"
	...
	"github.com/lasthyphen/dijetsgogo/api/keystore"
	...
	"github.com/lasthyphen/dijetsgogo/utils/hashing"
	...
	"github.com/lasthyphen/dijethh/eth"
	"github.com/lasthyphen/dijethh/params"
	"github.com/lasthyphen/dijethh/rpc"

	accountKeystore "github.com/lasthyphen/dijethh/accounts/keystore"
)
...
// BuildGenesisTest returns the genesis bytes for Dijethh VM to be used in testing
...
func addUTXO(sharedMemory *atomic.Memory, ctx *snow.Context, txID ids.ID, index uint32, assetID ids.ID, amount uint64, addr ids.ShortID) (*djtx.UTXO, error) {
	utxo := &djtx.UTXO{
		UTXOID: djtx.UTXOID{
			TxID:        txID,
			OutputIndex: index,
...
func GenesisVMWithUTXOs(t *testing.T, finishBootstrapping bool, genesisJSON string, configJSON string, upgradeJSON string, utxos map[ids.ShortID]uint64) (chan engCommon.Message, *VM, manager.Manager, *atomic.Memory, *engCommon.SenderTest) {
	...
	for addr, djtxAmount := range utxos {
		...
		if _, err := addUTXO(sharedMemory, vm.ctx, txID, 0, vm.ctx.DJTXAssetID, djtxAmount, addr); err != nil {
...
func TestBuildApricotPhase5Block(t *testing.T) {
	...
	txs := make([]*types.Transaction, 10)
	for i := 0; i < 10; i++ {
		tx := types.NewTransaction(uint64(i), key.Address, big.NewInt(10), 21000, big.NewInt(params.LaunchMinGasPrice*3), nil)
		signedTx, err := types.SignTx(tx, types.NewEIP155Signer(vm.chainID), key.PrivateKey)
		if err != nil {
			t.Fatal(err)
		}
...
func TestBuildBlockDoesNotExceedAtomicGasLimit(t *testing.T) {
	...
	for i := 0; i < mempoolTxs; i++ {
		utxo, err := addUTXO(sharedMemory, vm.ctx, txID, uint32(i), vm.ctx.DJTXAssetID, importAmount, testShortIDAddrs[0])
		assert.NoError(t, err)

		importTx, err := vm.newImportTxWithUTXOs(vm.ctx.XChainID, testEthAddrs[0], initialBaseFee, kc, []*djtx.UTXO{utxo})
		if err != nil {
			t.Fatal(err)
		}
...
func TestExtraStateChangeAtomicGasLimitExceeded(t *testing.T) {
	...
	// in ApricotPhase5.
	for i := 0; i < 100; i++ {
		_, err := addUTXO(sharedMemory1, vm1.ctx, txID, uint32(i), vm1.ctx.DJTXAssetID, importAmount, testShortIDAddrs[0])
		assert.NoError(t, err)

		_, err = addUTXO(sharedMemory2, vm2.ctx, txID, uint32(i), vm2.ctx.DJTXAssetID, importAmount, testShortIDAddrs[0])
		assert.NoError(t, err)
	}
