...
// Note: channel through which dijethh mempool push txs to vm is injected here
// to ease up UT, which target only VM behaviors in response to dijethh mempool
// signals
func TestMempoolEthTxsAddedTxsGossipedAfterActivation(t *testing.T) {
	...
	for _, err := range errs {
		assert.NoError(err, "failed adding dijethh tx to mempool")
	}
	...
	assert.NoError(errs[2], "failed adding dijethh tx to mempool")
...
func TestMempoolEthTxsAppGossipHandling(t *testing.T) {
	...
	// show that unknown dijethh hashes is requested
...
func TestMempoolEthTxsRegossipSingleAccount(t *testing.T) {
	...
	errs := vm.chain.GetTxPool().AddRemotesSync(ethTxs)
	for _, err := range errs {
		assert.NoError(err, "failed adding dijethh tx to remote mempool")
	}
...
func TestMempoolEthTxsRegossip(t *testing.T) {
	...
	for _, err := range errs {
		assert.NoError(err, "failed adding dijethh tx to local mempool")
	}
